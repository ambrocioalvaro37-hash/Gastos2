...
function renderEventsList(){
  ...
  eventos.filter(e=>!m||e.date.startsWith(m)).forEach(e=>{
    const li=document.createElement("li");li.className="item";
    li.innerHTML=`<div class="itemTop"><div><strong>${escapeHtml(e.description)}</strong><div class="muted">${e.date} ${e.time}</div></div><div class="actions"><button class="ghost" onclick="editEvento('${e.id}')">Editar</button><button class="ghost" onclick="deleteEvento('${e.id}')">Borrar</button></div></div>`;
    ul.appendChild(li);
  });
}
...
function renderIngresosList(){
  ...
  ingresos.filter(i=>!m||i.date.startsWith(m)).forEach(i=>{
    const li=document.createElement("li");li.className="item";
    li.innerHTML=`<div class="itemTop"><div><strong>${escapeHtml(i.description)}</strong><div class="muted">${i.date} • ${fmtARS(i.amount)}</div></div><div class="actions"><button class="ghost" onclick="editIngreso('${i.id}')">Editar</button><button class="ghost" onclick="deleteIngreso('${i.id}')">Borrar</button></div></div>`;
    ul.appendChild(li);
  });
}
