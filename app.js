...
function editEvento(id){
  const e=eventos.find(ev=>ev.id===id);if(!e)return;
  $("eventDate").value=e.date;$("eventTime").value=e.time||"";$("eventDescription").value=e.description;
  eventos=eventos.filter(ev=>ev.id!==id);saveEventos();
}
...
function editIngreso(id){
  ...
  ingresos=ingresos.filter(i=>i.id!==id);saveIngresos();renderBalance();
}
