...
const STORE_KEY="gastos_app_v1", SETTINGS_KEY="gastos_settings_v1", RATES_KEY="gastos_rates_cache_v1", METHODS_KEY="gastos_methods_v1", EVENTS_KEY="gastos_events_v1", INGRESOS_KEY="gastos_ingresos_v1";
const $=id=>document.getElementById(id);

// Estado
let gastos=loadGastos(), settings=loadSettings(), rates=loadRatesCache(), methods=loadMethods(), editingId=null;
let eventos=loadEventos(), ingresos=loadIngresos();
...
function init(){
  ...
  renderEventsList();
  renderIngresosList();};
  $("calToday").onclick=()=>{const t=new Date();calCurrent=new Date(t.getFullYear(),t.getMonth(),1);calSelectedISO=todayISO();renderCalendar();renderMonth();
  renderEventsList();
  renderIngresosList();renderDayDetails();};
  ...
  // Historial -> Mes
  $("btnHistoryToMonth").onclick=()=>{const m=$("filterMonth").value||monthISO(new Date());const [y,mo]=m.split("-").map(Number);calCurrent=new Date(y,mo-1,1);if(historyDayISO)calSelectedISO=historyDayISO;showView("month");renderCalendar();renderMonth();
  renderEventsList();
  renderIngresosList();renderDayDetails();};
  ...
  $("filterIngresoMonth").onchange=renderIngresosList;
...
function showView(which){
  ...
  if (which === "ingresos") $("tabIngresos").classList.add("active");
  
  if (which === "ingresos") renderBalance();
}
...
function renderMethodsManager(){
  ...
  ul.querySelectorAll("button[data-delmethod]").forEach(btn=>{
    btn.onclick=()=>{const name=btn.getAttribute("data-delmethod"); if(methodIsUsed(name)) return;
      methods=methods.filter(x=>x!==name); saveMethods(methods); renderMethodsSelect(); renderMethodsManager(); renderMonth();
  renderEventsList();
  renderIngresosList(); renderCalendar(); renderDayDetails(); renderList();
    };
  });
}
...
// ============ EVENTOS ============
function loadEventos(){const r=localStorage.getItem(EVENTS_KEY);if(!r)return[];try{return JSON.parse(r)||[];}catch{return[];}}
...
function onSaveEvent(){
  ...
  eventos.unshift({id:crypto.randomUUID(),date:d,time:t,description:desc,createdAt:new Date().toISOString()});
  saveEventos();$("eventDescription").value="";renderEventsList();alert("Guardado ✅");
}
function renderEventsList(){
  const m=$("filterEventMonth").value, list=eventos.filter(e=>m?e.date.startsWith(m):true);
  const ul=$("eventsList");ul.innerHTML="";
  list.forEach(e=>{
    const li=document.createElement("li");li.className="item";
    li.innerHTML=`<div class="itemTop"><div><strong>${escapeHtml(e.description)}</strong><div class="muted">${e.date} ${e.time||""}</div></div></div>`;
    ul.appendChild(li);
  });
}

// ============ INGRESOS ============
function loadIngresos(){const r=localStorage.getItem(INGRESOS_KEY);if(!r)return[];try{return JSON.parse(r)||[];}catch{return[];}}
function saveIngresos(){localStorage.setItem(INGRESOS_KEY,JSON.stringify(ingresos));}
function onSaveIngreso(){
  const d=$("ingresoDate").value||todayISO(), amt=Number($("ingresoAmount").value||0), desc=($("ingresoDesc").value||"").trim();
  if(!amt||amt<=0)return alert("Poné un monto válido");
  ingresos.unshift({id:crypto.randomUUID(),date:d,amount:amt,description:desc,createdAt:new Date().toISOString()});
...
function renderIngresosList(){
  const m=$("filterIngresoMonth").value, list=ingresos.filter(i=>m?i.date.startsWith(m):true);
  const ul=$("ingresosList");ul.innerHTML="";
  list.forEach(i=>{
    const li=document.createElement("li");li.className="item";
    li.innerHTML=`<div class="itemTop"><div><strong>${escapeHtml(i.description)}</strong><div class="muted">${i.date} • ${fmtARS(i.amount)}</div></div></div>`;
    ul.appendChild(li);
  });
}
